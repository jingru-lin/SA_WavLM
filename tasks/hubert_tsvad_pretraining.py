...
class HubertPretrainingConfig(FairseqDataclass):
    ...
    # Speaker directory
    spk_embedding_dir: str = field(
        default='',
        metadata={
            "help": "Directory that stores speaker embeddings"
        }
    )
    ...
    # Mixing specific fields
    noise_apply_prob: float = field(
        default=0.1,
        metadata={
            "help": "Control the portion of noisy and mixture data"
        },
    )
    ...
    get_multiple_prob: float = field(
        default=0.95,
        metadata={"help": "Control the probability of getting multiple noise audios for a single audio"}
    )


@register_task("hubert_tsvad_pretraining", dataclass=HubertPretrainingConfig)
class HubertTSVADPretrainingTask(FairseqTask):
    ...
    def load_dataset(self, split: str, **kwargs) -> None:
        ...
        manifest_noise = f"{self.cfg.data}/noise.tsv"
        spk_embedding_dir = f"{self.cfg.spk_embedding_dir}/{split}"

        dicts = [self.target_dictionary] if self.cfg.fine_tuning else self.dictionaries
        pad_list = [dict.pad() for dict in dicts] # [1]
        eos_list = [dict.eos() for dict in dicts] # [2]
        sil_list = [dict.sil() for dict in dicts] # [4]
        procs = [LabelEncoder(dict) for dict in dicts]
        # paths = [self.get_label_dir() + '/km_labels']
        # assert len(paths) == 1, "Currently support only 1 label files directory"
        paths = [f"{self.get_label_dir()}/{split}.{l}" for l in self.cfg.labels]

        # hubert v1: pad_audio=True, random_crop=False;
        self.datasets[split] = HubertTSVADDataset(
            manifest,
            ...
            spk_embedding_dir=spk_embedding_dir,
            spk_dim=self.cfg.speaker_dim,
            # mixing specific arguments
            get_multiple_prob=self.cfg.get_multiple_prob,
