...
class TransformerEncoder(nn.Module):
    ...
    def extract_features(
        ...
        x_conv = x_conv.transpose(1, 2) # B x T x C
        x = x + x_conv # B x T x C
        ...
        # x_spk1 = x.clone()
        # x_spk2 = x.clone()
        # go through first N layers separately
        # speaker 1
        for i, layer in enumerate(self.layers):
            dropout_probability = np.random.random()
            if not self.training or (dropout_probability > self.layerdrop) or i in self.speaker_injection_layers: # do not drop speaker layer
                if i not in self.speaker_injection_layers:
                    x_spk1, z, pos_bias = layer(
                        # x_spk1,
                        x,
        ...
        #             )
        #
        # # concatenate and go through last N layers
        # x = torch.cat([x_spk1, x_spk2], dim=-1)
...
class TransformerSentenceEncoderLayer_1(nn.Module):
    ...
    def __init__(
        ...
        # layer norm associated with the self attention layer
        # self.self_attn_layer_norm = LayerNorm(self.embedding_dim)
        self.self_attn_layer_norm = CondLayerNorm(self.embedding_dim, embed_dim=speaker_dim)
        ...
        # layer norm associated with the position wise feed-forward NN
        # self.final_layer_norm = LayerNorm(self.embedding_dim)
        self.final_layer_norm = CondLayerNorm(self.embedding_dim, embed_dim=speaker_dim)
